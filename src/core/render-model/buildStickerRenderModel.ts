...
import { buildBackgroundNodes, buildChromeLinks, buildHeaderNodes } from './chrome';
import { buildPageContent } from './pageBuilders';
import { getLayoutTarget } from './selectors';

function buildStickerOnlyPlan(config: PlannerConfig): PlannerDocumentPlan {
  const plan = buildPlannerPlan(config);
  const stickerPages = plan.pages
    .filter((page) => page.sectionType === 'stickers')
    .map((page, index) => ({
      ...page,
      pageNumber: index + 1,
    }));
